...
The signing-in Entra user, or a group they belong to, must also have `Virtual Machine Administrator Login` on the Linux jumpbox VM or Bastion SSH authentication will be rejected.
...
- VM RBAC granting `Virtual Machine Administrator Login` on the Linux jumpbox VM for configured Entra users or groups
...
- `Virtual Machine Administrator Login` RBAC on the Linux jumpbox VM for the specific signing-in user, or for a group they belong to, normally granted through `vm_admin_login_principal_ids`
...
Before running the proxy, make sure the Entra user you signed in with, or one of their Entra groups, has `Virtual Machine Administrator Login` on the Linux jumpbox VM.
...
- Add developer or group object IDs to `vm_admin_login_principal_ids` so the actual signing-in Entra user has `Virtual Machine Administrator Login` on the Linux jumpbox VM.
- By default, the jumpbox VM also grants `Virtual Machine Administrator Login` to `DO_PuC_Azure_Live_b9cee3_Owners` and `DO_PuC_Azure_Live_b9cee3_Contributors` by resolving those Entra group display names in Terraform.
...
- VM login is authorized by Entra ID RBAC, specifically `Virtual Machine Administrator Login` on the Linux jumpbox VM.
...
| Access denied to VM | Confirm the Entra user you signed in with, or one of their groups, has `Virtual Machine Administrator Login` on the Linux jumpbox VM through `vm_admin_login_principal_ids`, then re-run Terraform if needed. |
